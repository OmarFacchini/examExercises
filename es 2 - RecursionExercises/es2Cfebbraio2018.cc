...
For example, given the string "11Marzo2014", the returned string will be "112014".
...
char *estraiRecursive(const char myString[], int indexMyString, char myResult[], int indexMyResult){
    ...
    if((myString[indexMyString] == '\0') || (indexMyString == DIM - 1)){
        ...
        return myResult;
    }else if(myString[indexMyString] >= '0' && myString[indexMyString] <= '9'){ //looking for what the text asks
